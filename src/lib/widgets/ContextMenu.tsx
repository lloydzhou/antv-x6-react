...
import GraphContext, { ContextMenuContext, ContextMenuType } from '../GraphContext';
...
export const useContextMenu = (props: any) => {
  ...
  const handleShow = ({ e, x, y }: {e: MouseEvent, x: number, y: number}) => {
    e.preventDefault();
    e.stopPropagation();
    ...
    if (bindType === 'node' || bindType === 'edge') {
      ...
      if (views.length) {
        // @ts-ignore
        const v = views.sort((a, b) => a.cell.getZIndex() < b.cell.getZIndex()).pop()
        view = v
        item = v?.cell
      }
    }
  ...
  const handleClose = () => {
    setContext({ ...context, visible: false, x: 0, y: 0, view: null, item: null })
  }
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      // 卸载的时候恢复之前的值
      const { preventDefaultContextMenu = true, preventDefaultBlankAction = true } = oldOptions
